...
import { vacancies, candidates, timelineEntries } from './schema.ts';

function seed() {
  // Drop existing data
  rawDb.exec('DELETE FROM timeline_entries');
  rawDb.exec('DELETE FROM candidates');
  rawDb.exec('DELETE FROM vacancies');
  ...
  db.insert(candidates)
    .values([
      ...
      },
      {
        ...
        nextInterview: '2026-05-12 14:00',
        score: 80,
        location: 'Ramat Gan',
  ...
  console.log('✓ Seeded recruitment.db with vacancies, candidates, and timeline_entries');
}
